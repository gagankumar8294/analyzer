...
import { 
  buildAnalysisPrompt, 
  buildCompetitorDeepPrompt, 
  buildCalendarDeepPrompt 
 ...
 * Merges calculated engagement metrics with AI-synthesized SWOT and brand profiles.
 * Excludes heavy competitor detail templates and calendars to stay within token quotas.
 */
export async function runAIProfileAnalysis(profile: InstagramProfile, posts: Post[]): Promise<AnalysisResult> {
  ...
  try {
    ...
    return {
      ...
      calendar: [], // empty on initial core load, loaded on demand
...
/**
 * Executes competitor deep-dive audit using Gemini Flash.
 * Returns enriched profiles with templates, hooks, and growth tactics.
 */
export async function runAICompetitorAnalysis(profile: InstagramProfile, competitors: any[]): Promise<CompetitorData[]> {
  const geminiApiKey = process.env.GEMINI_API_KEY;
  if (!geminiApiKey || geminiApiKey === 'your_gemini_api_key_here') {
    return getFallbackCompetitorDetails(profile);
  }

  const prompt = buildCompetitorDeepPrompt(profile, competitors);

  try {
    const enriched = await generateJSON<CompetitorData[]>(prompt);
    if (!enriched || !Array.isArray(enriched)) {
      throw new Error('Received invalid enriched competitors format.');
    }
    return enriched;
  } catch (err: any) {
    console.error('Competitor deep-dive rate limit or API error. Returning fallback details:', err);
    return getFallbackCompetitorDetails(profile);
  }
}

/**
 * Generates 30-day content calendar on demand.
 */
export async function runAICalendarAnalysis(profile: InstagramProfile, insights: any): Promise<CalendarDay[]> {
  const geminiApiKey = process.env.GEMINI_API_KEY;
  if (!geminiApiKey || geminiApiKey === 'your_gemini_api_key_here') {
    return getFallbackCalendar(profile, insights);
  }

  const prompt = buildCalendarDeepPrompt(profile, insights);

  try {
    const calendar = await generateJSON<CalendarDay[]>(prompt);
    if (!calendar || !Array.isArray(calendar)) {
      throw new Error('Received invalid calendar format.');
    }
    return calendar;
  } catch (err: any) {
    console.error('Calendar generation rate limit or API error. Returning fallback calendar:', err);
    return getFallbackCalendar(profile, insights);
  }
}
 ...
 * Fallback helpers
 */
function getFallbackCompetitorDetails(profile: InstagramProfile): CompetitorData[] {
...
function getFallbackCalendar(profile: InstagramProfile, insights: any): CalendarDay[] {
  const days: CalendarDay[] = [];
  const formats: ('REEL' | 'CAROUSEL' | 'POST' | 'STORY')[] = ['REEL', 'CAROUSEL', 'POST', 'STORY'];
  const pillars = insights.contentPillars || ['Tips', 'Behind the scenes', 'Growth'];
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    days.push({
      ...
      contentType: fmt,
      theme: pillar,
      idea: `Value post number ${i} about ${pillar}`,
      hook: `Did you know this about ${pillar}?`,
      caption: `Here are the top things to consider for ${pillar}. What are your thoughts?`,
      hashtags: ['#instagramgrowth', '#contentstrategy', '#creators'],
      cta: 'Comment below with your thoughts!',
      script: fmt === 'REEL' ? 'Hook: Watch this. Body: Do X, Y, Z. CTA: Follow for more.' : undefined,
      thumbnailText: fmt === 'REEL' ? `Secret ${pillar} Hack` : undefined
    });
  }
